...
const ImageUploaderUseRef = () => {
  // 숨겨진 input 요소에 접근하기 위한 ref
  const inputImageRef = useRef<HTMLInputElement | null>(null)
  // 선택된 파일 데이터를 자장하는 ref
  ...
  const onChangeImage = (e: React.ChangeEvent<HTMLInputElement>) => {
    console.log("onChangeImage = " + JSON.stringify(e.target.files))
    ...
    if (files !== null && files.length > 0) {
      ...
      console.log("onChangeImage fileRef.current = " + JSON.stringify(fileRef.current))
    }
  }

  // 업로드 버튼이 클릭됐을때 호출된느 콜백
  ...
  return (
    <div>
      ...
      />
      <br></br>
      <button onClick={onClickUpload}>업로드 버튼</button>
      {message !== null && <p>{message}</p>}
    </div>
  )
}

export default ImageUploaderUseRef
